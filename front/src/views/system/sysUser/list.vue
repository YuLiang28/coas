...
import userRoleApi from "@/api/system/sysUserRole";
...
export default {
    ...
    methods: {
        ...
        // 根据id删除数据
        removeDataById(id) {
            this.$confirm(`此操作将永久删除id为 ${id} 记录, 是否继续?`, "提示", {
        ...
        getRoles() {
            userRoleApi.getRoles(this.sysUser.id).then(response => {
        ...
        assignRole() {
            ...
            userRoleApi.assignRoles(assginRoleVo).then(response => {
        ...
        switchStatus(row) {
            ...
            userApi.updateStatus(row.id, row.status).then(response => {
                if (response.code) {
                    this.$message.success(response.message || '切换用户状态成功')
                    this.dialogVisible = false
                    this.fetchData()
                }
            })
